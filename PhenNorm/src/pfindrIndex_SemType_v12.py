...
def mapping(str1, Text):
	...
	ExList1 = ['C1556084','C0596070','C0596227','C0687744','C1257890','C1705429','C0043210','C0086287','C0086582','C0683983','C0085756', 'C0687744','C1257890', 'C1705429', 'C0034510', 'C0019576', 'C0086528', 'C0086409', 'C0599755', 'C0015031','C0026193','C0239307','C1535514','C0027361']
	ExList2 = ['marital', 'married', 'unmarried', 'single', 'separated', 'engaged', 'divorced', 'widowed', 'widow', 'widower', 'domestic partnership', 'partner', 'cohabit', 'civil union']
		...
		for Sent2 in Phrase[1:]:
			...
			for item in CandidateList[1:]:
				...
				if mapList(SemType, EventList)==1:
					...
					# Replace ';' to avoid error due to ';' is a separator for each Event_PCN
					CandidatePreferred = CandidatePreferred.replace(';',' ')
				...
				# Remove class "podg" from SOI
				elif (SemType.find('famg')>=0 or SemType.find('popg')>=0) and (mapList(CandidateCUI, ExList1)==0) and (mapList(CandidatePreferred,RaceList)==0) and (not CandidatePreferred in ModelMap[UttText,'SOI_PCN']):
...
#        Str1 = Str1.replace('female','')
   ...
   # Rule 11: If string match 'onset' then convert into 'started' 
    ...
    # Remove "," for clean, applied when determine similar variables
    Str1 = Str1.replace(',',' ')
...
def readinput3(text_input):
	...
	cmd='echo "' + phenDesc + '"|/data/resources/MetaMap/public_mm/bin/metamap11v2 -q|' + """ sed 's/\\\\'"'"s'//g' """ + ' > ./tmp123'
	print cmd
	os.system(cmd)

	text1 = text_input
	
	readinput(text1, './tmp123')		
...
def main():
	...
	#AbbreFile = "/home/sodoan/pfindr/Normalization/Abbreviation/Abbr_consolidated_final.txt_modified"
	LoadAbbre(AbbreFile)
