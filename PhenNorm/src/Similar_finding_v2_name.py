...
def cluster1(DescIdx,Group1,List1):
	...
	for i in range(0,len(Group1)):
		for j in range(i+1,len(Group1)):
			...
			if 2>1:
				...
				# ----
				#tempL1S = Group1[i][DescIdx + 6].strip().replace(',',';').split(';')
				#tempL2S = Group1[j][DescIdx + 6].strip().replace(',',';').split(';')

				#TopicSTT1S = set(tempL1S)
				#TopicSTT2S = set(tempL2S)
				
				#InterTopicSTTS = list(TopicSTT1S&TopicSTT2S)

				#print InterTopicSTTS
				# ---
				...
				#JoinListLS = list(set(InterTopicSTTS)&set(List1))
				#JoinListS = set(InterTopicSTTS)&set(List1)
				#JoinListStrS = ':'.join(list(set(InterTopicSTTS)&set(List1))[0:])
				...
				if len(JoinList)>0:
					...
					TopicPCN1L = Group1[i][DescIdx + 6].strip().split(';')
	...
	for key in Cluster.keys():
		...
		for item in Cluster[key]:
			item1=item[DescIdx + 6].split(';')
			item2=item[DescIdx + 7].split(';')
			x=0
			for temp2i in item2:
				temp1[temp2i.strip()]=item1[x]
				x=x+1

		key1 = key.split(':')
		keyS = ''
		for key1i in key1:
			keyS = keyS + temp1[key1i] + ';'

		print key + '\t' + keyS
		
		for item in Cluster[key]:
			#print item[0] + '\t' + item[DescIdx]  + '\t' + item[DescIdx + 7] + '\t' + item[DescIdx + 8] 
			print '\t' + item[0] + '\t' + item[DescIdx] + '\t' + item[DescIdx + 6]  + '\t' + item[DescIdx + 7] + '\t' + item[DescIdx + 8] 
		print "==============="
...
def cluster(DescIdx,Group1):
	...
	for key in Cluster.keys():
		print key
		for item in Cluster[key]:
			#print '\t' + item[0] + "\t" + item[DescIdx + 6] + '\t' + item[DescIdx] + '\t' + item[DescIdx + 3] 
			print '\t' + item[0] + '\t' + item[DescIdx] + '\t' + item[DescIdx + 6]  + '\t' + item[DescIdx + 7] + '\t' + item[DescIdx + 8] 
		print "==============="
...
def cluster_keywords(DescIdx,Group1,WordList):
	...
	for key in Cluster.keys():
		if len(Cluster[key])>1:
			...
			for item in Cluster[key]:
				#print "\t" + item[DescIdx] 
				#print "\t" + item[0] + "\t" + item[DescIdx + 6] + '\t' + item[DescIdx] 
				print '\t' + item[0] + '\t' + item[DescIdx] + '\t' + item[DescIdx + 6]  + '\t' + item[DescIdx + 7] + '\t' + item[DescIdx + 8] 
		print "==============="
